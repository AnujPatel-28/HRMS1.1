-- Wires attendance_check_impossible_travel (20260903121818) into both punch paths.
--
-- The result is MERGED into verification_snapshot under `server_travel_check`, alongside the
-- geofence evidence, rather than replacing the client's snapshot -- the client still contributes
-- facts it alone knows (camera state, GPS error reason, work mode as the browser saw it).
--
-- Advisory only. Nothing raises on an implausible result: see the reasoning in 20260903121818.
-- A flag is for HR to look at, not a reason to refuse someone their attendance.
--
-- Both bodies derived from pg_get_functiondef() with two edits each.
...
BEGIN
  ...
  v_travel := public.attendance_check_impossible_travel(p_tenant_id, v_employee_id, p_lat, p_lng, v_now);
  ...
  INSERT INTO public.attendance (
    ...
    p_ip, v_loc.confidence, coalesce(v_loc.remote_exception_id, p_remote_exception_id), coalesce(p_verification_snapshot, '{}'::jsonb) || jsonb_build_object('server_travel_check', v_travel)
  )
...
BEGIN
  ...
  IF v_in !~ 'attendance_check_impossible_travel' OR v_out !~ 'attendance_check_impossible_travel' THEN
    RAISE EXCEPTION 'assertion: a punch path does not run the travel check';
  END IF;
  IF v_in !~ 'server_travel_check' OR v_out !~ 'server_travel_check' THEN
    RAISE EXCEPTION 'assertion: the travel result is not recorded in the evidence';
  END IF;

  -- everything the previous migrations established must still hold
  IF v_in !~ 'attendance_evaluate_location' OR v_out !~ 'attendance_evaluate_location' THEN
    RAISE EXCEPTION 'assertion: server-side location evaluation was lost';
  END IF;
  IF v_in !~ 'GEOFENCE_BLOCKED' OR v_out !~ 'GEOFENCE_BLOCKED' THEN
    RAISE EXCEPTION 'assertion: geofence enforcement was lost';
  END IF;
  IF v_in !~ 'selfie_missing' OR v_out !~ 'selfie_missing' THEN
    RAISE EXCEPTION 'assertion: the selfie_missing carve-out was lost';
  END IF;
  IF v_out !~ 'short_break_limit_minutes' OR v_out !~ 'PAYROLL_LOCKED' THEN
    RAISE EXCEPTION 'assertion: punch_out break or payroll-lock handling was lost';
  END IF;
END;
$assert$;
