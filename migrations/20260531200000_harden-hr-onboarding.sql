-- Migration: Harden HR Onboarding Flow (Unique Constraints & RPC Update)
-- Description: Adds a unique constraint on (tenant_id, employee_code) and updates the HR password reset RPC to allow claiming auth users before employee rows exist.

-- 1. Add unique constraint for employee_code per tenant
-- (This ensures two HRs cannot submit the same employee code concurrently for the same tenant)
CREATE UNIQUE INDEX IF NOT EXISTS employees_tenant_employee_code_unique
ON public.employees (tenant_id, employee_code);

-- 2. Update set_employee_password_by_hr RPC
