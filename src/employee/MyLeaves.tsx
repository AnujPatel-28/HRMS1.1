...
export default function MyLeaves() {
  ...
  const [form, setForm] = useState({ leave_type: "casual" as typeof LEAVE_TYPES[number], start_date: "", end_date: "", reason: "" });
  ...
  const approvedLeaves = leaves.filter(l => l.status === "approved" && l.start_date.startsWith(thisYear));
  const casualTaken = approvedLeaves.filter(l => l.leave_type === "casual").reduce((s, l) => s + (l.total_days ?? 0), 0);
  const sickTaken = approvedLeaves.filter(l => l.leave_type === "sick").reduce((s, l) => s + (l.total_days ?? 0), 0);
    ...
    <section className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm">
          <p className="text-xs font-medium text-slate-500">Casual Taken ({thisYear})</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">{casualTaken}</p>
          <p className="text-xs text-slate-400">days</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm">
          <p className="text-xs font-medium text-slate-500">Sick Taken ({thisYear})</p>
          <p className="text-2xl font-bold text-amber-600 mt-1">{sickTaken}</p>
          <p className="text-xs text-slate-400">days</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm sm:col-span-1 col-span-2">
          <p className="text-xs font-medium text-slate-500">Total Pending</p>
          <p className="text-2xl font-bold text-rose-600 mt-1">{leaves.filter(l => l.status === "pending").length}</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm max-w-lg">
          ...
          <form onSubmit={applyLeave} className="space-y-4">
            <div>
              ...
              <select value={form.leave_type} onChange={e => setForm({...form, leave_type: e.target.value as any})}
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none ring-brand-600 focus:ring capitalize">
            ...
            {totalDays > 0 && (
              <div className="rounded-lg bg-brand-50 border border-brand-200 px-3 py-2 text-sm font-semibold text-brand-700">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={l.id} className="hover:bg-slate-50 transition">
                  <td className="px-4 py-3 capitalize font-medium text-slate-800">{l.leave_type}</td>
                  ...
                  <td className="px-4 py-3">
                    {l.status === "pending" && (
                      <button onClick={() => setCancelModal({ isOpen: true, leaveId: l.id, dates: `${l.start_date} to ${l.end_date}`, type: l.leave_type ?? "Leave" })}
