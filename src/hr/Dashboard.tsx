...
export default function HRDashboard() {
  ...
  const context = useOutletContext<{ layoutStyle?: 'dropdown' | 'double_sidebar' | 'classic' }>() || {};
  ...
  const [extendingEmpId, setExtendingEmpId] = useState<string | null>(null);
  ...
  const fetchAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [empRes, leaveRes, attRes, notifRes, shiftRes, empShiftRes, probationRes] = await Promise.all([
      ...
      if (probationRes.error) throw probationRes.error;
      ...
      setProbationEmployees((probationRes.data ?? []) as Employee[]);
    } catch (err) {
    ...
    try {
      const { error } = await db
        ...
        .update({
          ...
          employment_confirmed_at: new Date().toISOString()
        })
        .eq("id", empId);
      if (error) throw error;
      toastSuccess("Employment confirmed successfully.");
      void fetchAll();
    } catch (err) {
      toastError("Failed to confirm employment.");
    } finally {
      setSubmittingProbation(false);
    }
  };

  const handleExtendProbation = async (empId: string) => {
    if (!extensionDate) {
      toastError("Please select a new end date.");
      return;
    }
    setSubmittingProbation(true);
    try {
      const { error } = await db
        ...
        .eq("id", empId);
      if (error) throw error;
      toastSuccess("Probation period extended successfully.");
      setExtendingEmpId(null);
      setExtensionDate("");
      void fetchAll();
    } catch (err) {
      toastError("Failed to extend probation.");
    } finally {
      setSubmittingProbation(false);
    }
  };
  ...
  if (currentLayout === "classic") {
    return (
      <section className="space-y-6">
            ...
            {kpis.map((k) => {
              ...
              return (
                <div key={k.label} className="h-full">
                  ...
                  <div className="relative h-full overflow-hidden rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_8px_30px_rgba(0,0,0,0.04)] flex flex-col justify-between lg:hidden">
        ...
        {/* Probation Period Ending Alert Widget */}
        <div className="grid gap-5 lg:grid-cols-2 mt-6 mb-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl -translate-y-1">
            <h3 className="mb-4 font-semibold text-slate-800 flex items-center gap-2">
              <Clock className="h-5 w-5 text-amber-500" />
              <span>Probation Reviews Ending Soon</span>
            </h3>
            {loading ? (
              <div className="space-y-3">
              ...
              <div className="py-4">
                <EmptyState icon={CheckCircle2} title="All confirmed!" description="No employees on probation ending soon." />
              </div>
            ) : (
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {probationEmployees.map((emp) => {
                  const isExtending = extendingEmpId === emp.id;
                  const daysLeft = emp.probation_end_date
                    ? Math.ceil((new Date(emp.probation_end_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
                    : null;
                        ...
                        {isExtending ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="rounded-lg border border-slate-300 px-2 py-1 text-xs outline-none bg-white text-slate-900"
                            />
                            <button
                              disabled={submittingProbation}
                              onClick={() => handleExtendProbation(emp.id)}
                              className="bg-brand-600 hover:bg-brand-700 text-white font-bold px-2 py-1 rounded text-xs"
                            >
                              Save
                            </button>
                            <button
                              onClick={() => { setExtendingEmpId(null); setExtensionDate(""); }}
                              className="text-slate-400 hover:text-slate-650 text-xs px-1"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <>
                            <button
                              disabled={submittingProbation}
                              onClick={() => { setExtendingEmpId(emp.id); setExtensionDate(emp.probation_end_date || ""); }}
                              className="px-2.5 py-1 text-xs font-semibold border border-slate-300 bg-white hover:bg-slate-50 rounded-lg text-slate-700 transition"
                            >
                              Extend
                            </button>
                            <button
                              ...
                              className="px-2.5 py-1 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 rounded-lg text-white transition shadow-sm"
                            >
                              Confirm
                            </button>
                          </>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      ...
      <div className="lg:col-span-2 space-y-6">
        {/* Welcome Banner */}
        <div
          className="rounded-2xl border border-brand-100 p-6 text-white shadow-md bg-cover bg-center bg-no-repeat bg-[url('/full.svg')] min-[912px]:bg-[url('/green%20gradiant.svg')] relative overflow-hidden"
          style={{ backgroundColor: "#059669" }}
        >
          <div className="relative z-10 w-full md:w-2/3">
            <p className="text-sm font-medium opacity-80">Good {new Date().getHours() < 12 ? "morning" : new Date().getHours() < 17 ? "afternoon" : "evening"},</p>
        ...
        {/* Overview Section */}
        <div className="flex flex-col gap-4 sm:gap-6 lg:bg-white lg:rounded-3xl lg:border lg:border-slate-100 lg:shadow-[0_8px_30px_rgba(0,0,0,0.02)] lg:p-6">
          {/* Overview Header & Shift Selector */}
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:px-2 lg:px-0">
            <h3 className="text-lg font-bold text-slate-900">Today's Overview</h3>
            ...
            {kpis.map((k) => {
              ...
              return (
                <div key={k.label} className="h-full">
                  ...
                  {/* DESKTOP VIEW (Hidden below lg) */}
                  <div className="hidden lg:flex relative overflow-hidden h-full rounded-2xl border border-slate-100 bg-white p-5 lg:min-h-[160px] flex-col justify-between transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 duration-200">
                    <div className={`absolute -right-10 -top-10 h-32 w-32 rounded-full blur-3xl opacity-20 ${glowClass}`} />
                    <div className="relative z-10 flex items-center justify-between mb-2">
                      <p className="text-sm font-medium text-slate-500">{k.label}</p>
                      <span className={`rounded-xl p-2 ${k.bg} ${k.color}`}>{k.icon}</span>
        ...
        {/* Sub-grid for Leaves & Probation */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Pending Leave Requests */}
          <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.02)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-semibold text-slate-800">Pending Leave Requests</h3>
              <button
                onClick={() => navigate("/hr/leaves")}
                        ...
                        {isExtending ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              value={extensionDate}
                              onChange={(e) => setExtensionDate(e.target.value)}
                              className="rounded-lg border border-slate-300 px-2 py-1 text-[11px] outline-none bg-white text-slate-900"
                            />
                            <button
                              disabled={submittingProbation}
                              onClick={() => handleExtendProbation(emp.id)}
                              className="bg-brand-600 hover:bg-brand-700 text-white font-bold px-2 py-1 rounded text-[11px]"
                            >
                              Save
                            </button>
                            <button
                              onClick={() => { setExtendingEmpId(null); setExtensionDate(""); }}
                              className="text-slate-400 hover:text-slate-650 text-[11px] px-1"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <>
                            <button
                              disabled={submittingProbation}
                              onClick={() => { setExtendingEmpId(emp.id); setExtensionDate(emp.probation_end_date || ""); }}
                              className="px-2 py-0.5 text-[11px] font-semibold border border-slate-300 bg-white hover:bg-slate-50 rounded-lg text-slate-700 transition"
                            >
                              Extend
                            </button>
                            <button
                              disabled={submittingProbation}
                              onClick={() => handleConfirmProbation(emp.id)}
                              className="px-2 py-0.5 text-[11px] font-semibold bg-emerald-600 hover:bg-emerald-700 rounded-lg text-white transition shadow-sm"
                            >
                              Confirm
                            </button>
                          </>
