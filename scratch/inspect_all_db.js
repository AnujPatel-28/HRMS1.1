...
const sql = `
  SELECT 'channels' AS type, pattern::text AS name, enabled::text AS col3, '' AS col4
  FROM realtime.channels
  UNION ALL
  SELECT 'triggers' AS type, trigger_name::text AS name, event_manipulation::text AS col3, action_statement::text AS col4
  FROM information_schema.triggers
  WHERE event_object_table IN ('posts', 'post_reactions')
  UNION ALL
  SELECT 'policies' AS type, policyname::text AS name, tablename::text AS col3, cmd::text AS col4
  FROM pg_policies
  WHERE schemaname = 'public' AND tablename IN ('posts', 'post_reactions')
  UNION ALL
  SELECT 'columns' AS type, table_name::text AS name, column_name::text AS col3, data_type::text AS col4
...
async function main() {
  console.log("Executing unified database metadata query...");
